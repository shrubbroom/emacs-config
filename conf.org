...
*** Open file in external applications
    ...
    Keybinding
*** Toggle proxy
    #+BEGIN_SRC emacs-lisp :tangle (when (boundp 'init/local-proxies) "yes")
...
*** Setup theme
    ...
    #+END_SRC
    Setup theme.
    #+BEGIN_SRC emacs-lisp :tangle (when (boundp 'init/theme) "yes")
...
*** Keybindings
    ...
    #+END_SRC
*** Define path
#+BEGIN_SRC emacs-lisp :tangle (when (fboundp 'init/define-path) "yes")
  (init/define-path)
#+END_SRC
*** Issues
    #+BEGIN_SRC emacs-lisp
