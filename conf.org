...
*** Prompt
    #+BEGIN_SRC emacs-lisp
      ...
      (setq eshell-prompt-function-dark #'(lambda nil
                                            (concat
                                             (propertize (user-login-name) 'face `(:foreground "#268bd2" :weight bold))
                                             (propertize " at " 'face `(:background (face-background 'default)))
                                             (propertize (system-name) 'face `(:foreground  "#268bd2" :weight bold))
                                             (propertize " in " 'face `(:background (face-background 'default)))
                                             (propertize (abbreviate-file-name (eshell/pwd)) 'face `(:foreground "#859900"
                                                                                                                 :weight bold :slant italic))
                                             (if (and (boundp 'url-proxy-services)
                                                      (assoc "http" url-proxy-services))
                                                 (concat
                                                  (propertize " via " 'face `(:background (face-background 'default)))
                                                  (propertize (cdr (assoc "http" url-proxy-services)) 'face `(:foreground "#2aa198"
                                                                                                                          :weight bold))))
                                             (propertize "\n" 'face `(:background (face-background 'default)))
                                             (propertize (if (= (user-uid) 0) "# " "λ") 'face `(:slant default :weight bold))
                                             (propertize " " 'face `(:background (face-background 'default))))))
...
#+BEGIN_SRC emacs-lisp
  (advice-add 'sort-tab-get-tab-name
              :around #'(lambda (origin buf current-buffer)
                          (let* ((name (buffer-name buf))
                                 (short-name (concat "roam:" (replace-regexp-in-string "\.org"
                                                                                       ""
                                                              (replace-regexp-in-string "20[0-9][0-9][01][0-9][0-3][0-9]+-"
                                                                                        "" name)))))
                            (if (eq (length name) (- (length short-name) 1))
                                (apply origin `(,buf ,current-buffer))
                                (propertize
                                 (format " %s "
                                         (let ((ellipsis "..."))
                                           (if (> (length short-name) sort-tab-name-max-length)
                                               (format "%s%s"
                                                       (substring short-name
                                                                  0
