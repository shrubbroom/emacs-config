      ...
      (setq eshell-prompt-regexp "^[#λ] "
            eshell-prompt-function #'(lambda nil
                                       (concat
                                        ...
                                        (propertize (abbreviate-file-name (eshell/pwd)) 'face `(:foreground "green" :weight bold :slant italic))
                                        (if (and (boundp 'url-proxy-services)
                                                 (assoc "http" url-proxy-services))
                                            (concat
                                             (propertize " via " 'face `(:background (face-background 'default)))
                                             (propertize (cdr (assoc "http" url-proxy-services)) 'face `(:foreground "cyan" :weight bold))))
